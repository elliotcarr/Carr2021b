...
% t = 0.2; fig_label = {'(a)','(b)','(c)'}; cnt = 21; w = @(y) zeros(size(y)); wd = @(y) zeros(size(y)); epsilon = 0.0;

% Produces Figure 2(d)(e)(f)
% t = 0.2; fig_label = {'(d)','(e)','(f)'}; cnt = 22; w = @(y) y; wd = @(y) 1; epsilon = 0.05;

% Produces Figure 2(g)(h)(i)
% t = 0.2; fig_label = {'(g)','(h)','(i)'}; cnt = 23; w = @(y) sin(pi*y); wd = @(y) pi*cos(pi*y); epsilon = 0.05;

% Produces Figure 2(j)(k)(l)
t = 0.2; fig_label = {'(j)','(k)','(l)'}; cnt = 24; w = @(y) sin(7*pi*y); wd = @(y) 7*pi*cos(7*pi*y); epsilon = 0.02;
% 
% fig = '3';
% Produces Figure 3(a)(b)(c)
% t = 0.2; fig_label = {'(a)','(b)','(c)'}; cnt = 31; w = @(y) sin(7*pi*y); wd = @(y) 7*pi*cos(7*pi*y); epsilon = 0.01;
% % 
% Produces Figure 3(d)(e)(f)
% t = 0.2; fig_label = {'(d)','(e)','(f)'}; cnt = 32; w = @(y) sin(7*pi*y); wd = @(y) 7*pi*cos(7*pi*y); epsilon = 0.02;
% % 
% Produces Figure 3(g)(h)(i)
% t = 0.2; fig_label = {'(g)','(h)','(i)'}; cnt = 33; w = @(y) sin(7*pi*y); wd = @(y) 7*pi*cos(7*pi*y); epsilon = 0.05;
...
QL = @(s) 0.*s; % boundary condition function at x = L
...
% Leading order term
[U1,U2] = leading_order_term(D1,D2,ell,L,C0,QL);
...
view(2)
if isequal(fig,'3')
    colorbar('Ticks',[-16:3:2],'YTickLabel',{'10^{-16}','10^{-13}','10^{-10}','10^{-7}','10^{-4}',...
        '10^{-1}','10^{2}'});
else
...
%% Save figs
addpath('/Users/carre/Dropbox/Documents/Research/Code/Other packages/export_fig-master2')
path_name = '../../Figures/';
name = 'Case';
if save_figs
